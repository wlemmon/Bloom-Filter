...
class HashType1(object):
    ...
    def __init__(self, config, genHashes):
        ...
        if genHashes :
            ...
            self.seeds = random.random_integers(0, 10000000, self.k).astype('int64')
            #print self.seeds
            #asdf
    ...
    def getHashList(self, x):
        ...
        if isinstance(x, np.ndarray):
            res = np.zeros((self.k, len(x)), dtype='int64')
            for ki in range(self.k):
                for xi in range(len(x)):
                    random.seed(self.seeds[ki]+xi)
                    res[ki, xi] = random.randint(self.n)
        else:
            for i in range(self.k):
                random.seed(self.seeds[i]+x)
                res.append(random.randint(self.n))
...
class HashType2(object):
    ...
    def __init__(self, config, genHashes):
        ...
        if genHashes :
            ...
            self.a = random.random_integers(0, 10000000, self.k)
            self.b = random.random_integers(0, 10000000, self.k)
            
        #if not gen (task 2), then use hashes that are in config dictionary
        else :
            self.a = np.array(config['a'], dtype='int64')
            self.b = np.array(config['b'], dtype='int64')
        
        self.P = util.findNextPrime(self.N)
        assert util.checkIfPrime(self.n)
    ...
    def getHashList(self, x):
        ...
        #your code goes here
        if isinstance(x, np.ndarray):
            res = (((self.a[:,None] * x + self.b[:, None] ) % self.P) % self.n)
        else:
            for i in range(self.k):
                res.append(((self.a[i] * x + self.b[i] ) % self.P) % self.n)
        return res
...
class BloomFilter(object):     
    ...
    def __init__(self, config):
        ...
        #your code goes here
        self.array = np.zeros((config['n'],), dtype='bool')
        #print self.array
    def add(self, x):
        ...
        #your code goes here
        self.array[self.hashFunc.getHashList(x)] = 1
    ...
    def contains(self, x):
        ...
        #your code goes here
        return np.all(self.array[self.hashFunc.getHashList(x)], axis=0)
...
def task1(configData):
    ...
    h = HashType1(configData,genHashes=True)
    print h.getHashList(100)
    
    h = HashType2(configData,genHashes=True)
    print h.getHashList(100)
...
def task3(configData):
    ...
    input = np.array(util.readIntFileDat('testInput.txt'), dtype='int64')
    
    
                                        
    for type in (2, 1):
        for c in (10, 15,):
            for k in map(int, [c * i for i in np.arange(0.4, 1.0, 0.1)]):
                
                
                falsePositiveRate = 0
                trials = 10
                for trial in range(trials):
                    print 'type', type, 'c', c, 'k', k, 'trial', trial
                    configData = {  'N': 2147483647,#2**32 - 1,
                                    'm': 100000,
                                    'n': 1917017,
                                    'k': k,
                                    'task': 3,
                                    'type': type,
                                    'genSeed':random.randint(1000000000)}#877623067
                    
                    bf = BloomFilter(configData)
                    
                    bf.add(input[0:configData['m']])
                    
                    assert np.all(bf.contains(input[:configData['m']]))
                    falsePositiveRate += np.sum(bf.contains(input[configData['m']:len(input)]))
                print 'falsePositiveRate', str(float(falsePositiveRate)/(trials * (len(input) - configData['m'])))
